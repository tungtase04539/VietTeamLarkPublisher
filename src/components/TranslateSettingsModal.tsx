...
const PROVIDER_LABELS: Record<ModelProvider, string> = {
    ...
    anthropic: 'Anthropic — Claude (API trực tiếp)',
    '302ai': '302.ai — proxy, 1 key dùng tất cả model',
    'openai-compatible': 'Custom OpenAI-compatible endpoint',
};

function PriceTag({ priceIn, priceOut }: { priceIn: number; priceOut: number }) {
    if (priceIn === 0 && priceOut === 0) return (
        <span className="text-[10px] font-semibold text-[#34c759] bg-[#34c759]/10 px-1.5 py-0.5 rounded-full">Miễn phí</span>
    );
    const formatPrice = (p: number) => p < 1 ? `$${p}` : `$${p.toFixed(0)}`;
    return (
        <span className="text-[10px] font-mono text-[#86868b] whitespace-nowrap">
            {formatPrice(priceIn)} / {formatPrice(priceOut)} /1M
        </span>
    );
}
...
export default function TranslateSettingsModal({ isOpen, onClose }: Props) {
    ...
    useEffect(() => { if (isOpen) setSettings(loadTranslateSettings()); }, [isOpen]);

    const up = <K extends keyof TranslateSettings>(k: K, v: TranslateSettings[K]) =>
        setSettings(s => ({ ...s, [k]: v }));

    const handleSave = () => { saveTranslateSettings(settings); onClose(); };
    ...
    const currentPreset = PRESET_MODELS[currentPresetIdx];
    const apiPlaceholder = currentPreset?.placeholder ?? 'API Key';
    const isDefaultGemini = settings.provider === DEFAULT_SETTINGS.provider
        && settings.model === DEFAULT_SETTINGS.model && !settings.apiKey;
                    ...
                    >
                        <div className="w-full max-w-lg bg-white dark:bg-[#1c1c1e] rounded-2xl shadow-2xl border border-[#00000012] dark:border-[#ffffff12] overflow-hidden">
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-b border-[#00000010] dark:border-[#ffffff10]">
                                <div>
                                    <h2 className="text-[15px] font-semibold text-[#1d1d1f] dark:text-[#f5f5f7]">Cài đặt dịch thuật</h2>
                                    <p className="text-[12px] text-[#86868b] mt-0.5">Settings lưu cục bộ trong trình duyệt</p>
                                </div>
                                <button onClick={onClose} className="p-2 rounded-full hover:bg-[#00000008] dark:hover:bg-[#ffffff10] transition-colors text-[#86868b]">
                                    <X size={16} />
                                </button>
                            </div>
                            ...
                            <div className="px-6 py-5 space-y-5">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[12px] font-medium text-[#86868b] uppercase tracking-wide">Model</label>
                                                ...
                                                <optgroup key={group} label={group}>
                                                    {PRESET_MODELS.map((p, i) => p.group === group ? (
                                                        <option key={i} value={i}>
                                                            {p.label}{p.priceIn > 0 ? ` — $${p.priceIn}/$${p.priceOut}/1M` : ' — Miễn phí'}
                                                        </option>
                                                    ) : null)}
                                                </optgroup>
                                            ))}
                                    ...
                                    {currentPreset && (
                                        <div className="flex items-start gap-2.5 px-3.5 py-2.5 rounded-xl bg-[#f5f5f7] dark:bg-[#2c2c2e]">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[12px] text-[#1d1d1f] dark:text-[#f5f5f7] font-medium">{currentPreset.label}</p>
                                                <p className="text-[11.5px] text-[#86868b] dark:text-[#a1a1a6] mt-0.5">{currentPreset.desc}</p>
                                            </div>
                                            <PriceTag priceIn={currentPreset.priceIn} priceOut={currentPreset.priceOut} />
                                        </div>
                                    )}

                                    {settings.provider === 'openai-compatible' && (
                                        <input value={settings.model} onChange={e => up('model', e.target.value)}
                                ...
                                {settings.provider === '302ai' && (
                                    <div className="space-y-1.5">
                                        <label className="text-[12px] font-medium text-[#86868b] uppercase tracking-wide flex items-center gap-2">
                                            302.ai API Key
                                            <span className="normal-case font-normal text-[#34c759] text-[11px]">• 1 key dùng Claude, GPT, Gemini, DeepSeek, Grok...</span>
                                        </label>
                                        <input type="password" value={settings.ai302Key} onChange={e => up('ai302Key', e.target.value)}
                                ...
                                {settings.provider === 'openai-compatible' && (
                                    <div className="space-y-1.5">
                                        ...
                                        <input value={settings.baseUrl} onChange={e => up('baseUrl', e.target.value)}
                                            placeholder="https://api.example.com/v1"
                                            className="w-full bg-[#f5f5f7] dark:bg-[#2c2c2e] border-0 rounded-xl px-4 py-3 text-[14px] font-mono text-[#1d1d1f] dark:text-[#f5f5f7] focus:outline-none focus:ring-2 focus:ring-[#0066cc]/30 placeholder-[#86868b]" />
                                    </div>
                                )}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    {showPrompt && (
                                        <div className="space-y-2">
                                            <textarea value={settings.customPrompt} onChange={e => up('customPrompt', e.target.value)}
                                                placeholder={DEFAULT_PROMPT} rows={8}
                                            ...
                                            {settings.customPrompt.trim() && (
                                                <button onClick={() => up('customPrompt', '')} className="text-[12px] text-[#86868b] hover:text-[#ff3b30] transition-colors">
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-t border-[#00000010] dark:border-[#ffffff10] bg-[#f5f5f7]/50 dark:bg-[#2c2c2e]/50">
                                <button onClick={handleReset} className="flex items-center gap-1.5 text-[13px] text-[#86868b] hover:text-[#ff3b30] transition-colors">
                                    <RotateCcw size={13} /> Về mặc định
                                </button>
                                <div className="flex gap-3">
                                    <button onClick={onClose} className="px-4 py-2 rounded-xl text-[13px] text-[#86868b] hover:bg-[#00000008] dark:hover:bg-[#ffffff10] transition-colors">Hủy</button>
